...
print_usage() {
  cat <<EOF
Usage:
  $0 [OBJ="name"] [ANGLES="0 22.5 45 67.5"] [TOL=0.05] [POLL=0.1] [NUM_EXPOSURES=1] [HWP_CYCLES=1] [XY="dx dy"]
...
if [[ "$DITHER" -eq 1 ]]; then
  # Expect XY to be something like "3 0"
  read -r DX DY <<< "$XY"
  if [[ -z "$DX" || -z "$DY" ]]; then
...
if [[ "$DITHER" -eq 0 ]]; then
  ...
  # Explicit negation of DX and DY for the return

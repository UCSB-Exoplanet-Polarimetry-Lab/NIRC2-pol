...
# --- set filter on remote (two commands, separately) ---
ssh nirc2eng@waikoko-new "filt $FILT" \
  || { echo "Error: failed to set filter to $FILT"; exit 1; }
ssh nirc2eng@waikoko-new "filt Wollaston" \
  || { echo "Error: failed to set Wollaston filter"; exit 1; }

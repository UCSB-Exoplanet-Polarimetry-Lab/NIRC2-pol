...
# Check to make sure Wollaston and field mask are in - aborts script otherwise
# Get current filter and mask values via ssh to NIRC2 computer
fwiname_return=$(ssh nirc2eng@waikoko-new "show -s nirc2 fwiname")
fwoname_return=$(ssh nirc2eng@waikoko-new "show -s nirc2 fwoname")
slmname_return=$(ssh nirc2eng@waikoko-new "show -s nirc2 slmname")

# Extract value after '=' and strip spaces
fwiname=$(echo "$fwiname_return" | awk -F'=' '{print $2}' | xargs)
fwoname=$(echo "$fwoname_return" | awk -F'=' '{print $2}' | xargs)
slmname=$(echo "$slmname_return" | awk -F'=' '{print $2}' | xargs)

# # Check values and abort if not correct
# if [[ "$fwoname" != "Wollaston" ]]; then
#   echo "Error: fwoname is '$fwoname', but must be 'Wollaston'. Aborting."
#   exit 1
# fi
# if [[ "$slmname" != "5arcsec_wide" ]]; then
#   echo "Error: slmname is '$slmname', but must be '5arcsec_wide'. Aborting."
#   exit 1
# fi

# --------------------
# Defaults (override with key=value args)
OBJ="test_pcu_rotation"          # object base name
...
print_usage() {
  cat <<EOF
Usage:
  $0 FILT=J|H|K [OBJ="name"] [ANGLES="0 45 22.5 67.5"] [TOL=0.05] [POLL=0.1] [NUM_EXPOSURES=1] [HWP_CYCLES=1]
...
echo "Running with:"
echo "  FWINAME          = $fwiname"
echo "  FWONAME          = $fwoname"
echo "  SLMNAME          = $slmname"
echo "  OBJ           = $OBJ"
echo "  ANGLES        = $ANGLES"
echo "  TOL           = $TOL"
echo "  POLL          = $POLL"
